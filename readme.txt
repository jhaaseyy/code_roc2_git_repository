...
This is the same code as the 22-Jul release but put onto roc2.2. Testing recent
changes with this system that contains the previous version of the interface
board. Testing not complete yet, waiting for availability of Zephyr simulator.

Fixed:

Changes:
- Modified makefile so that it always "make all" will erase previous binaries
  forcing a rebuild of all source files.
...
- Switching to LP mode doesn't shut down the GPS receiver when the system starts
  the roc modules automatically at boot up. This behavior does not appear when 
  launching the code manually from the command line with the -v flag. TBD whether
  the difference in behavior is due to the -v flag or launching automatically
  vs. manually.
